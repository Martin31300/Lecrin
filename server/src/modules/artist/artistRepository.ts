...
async function selectOne(id: number) {
  const [artist] = await db_client.query("SELECT * FROM artist WHERE id = ?",[id]);
  return artist;
}

async function create (newArtist: Artist) {
  const [result] = await db_client.query("INSERT INTO artist ? VALUES (?, ?, ?, ?) " ,
    [
      newArtist.name,
...
async function deleteById(id: number) {
  const [result] = await db_client.query<Result> ("DELETE FROM artist WHERE id = ?", [id]);
  return result;
}

async function updateById(artist: Partial <Artist> , id: number) {
  const [result] = await db_client.query ("UPDATE artist SET ? WHERE id = ?", [artist,id]);
  return result;  
}
